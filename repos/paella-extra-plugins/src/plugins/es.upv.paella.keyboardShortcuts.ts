import { Paella, isVolumeApiAvailable, type PopUpButtonPluginConfig } from '@asicupv/paella-core';
...
export const createKeyboardEventListener = (player: Paella, validPlaybackRates: number[] = [0.5, 0.75, 1, 1.25, 1.5, 2]) => {
  ...
  const decreaseSpeed = async () => {
    const currentPR = await player.playbackRate();
    let selected = 0;
  ...
  const toggleCaptions = async () => {
    if (player?.captionsCanvas?.captions?.length > 0) {
  ...
  return async (event: KeyboardEvent) => {
    ...
    switch (event.code) {
      ...
      case 'KeyM':
        event.preventDefault();
        const currentVolume = await player.volume();
      ...
      case 'KeyJ':
        event.preventDefault();
        const currentTimeJ = await player.currentTime();
      ...
      case 'KeyL':
        event.preventDefault();
        const currentTimeL = await player.currentTime();
        const durationL = await player.duration();
      ...
      case 'End':
        event.preventDefault();
        const durationE = await player.duration();
      ...
      case 'ArrowUp':
        if (!focused) {
          event.preventDefault();
          const volume = await player.volume();
