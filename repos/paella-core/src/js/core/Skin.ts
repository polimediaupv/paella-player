...
export async function checkLoadSkinIcons(this: Skin) {
    if (Array.isArray(this._skinData?.icons)) {
        await Promise.all(this._skinData.icons.map(({
            ...
            return new Promise(async (resolve: Function, reject: Function) => {
                ...
                if (div.children[0] && div.children[0].tagName === 'svg') {
                    ...
                    const iconFullUrl = this._skinUrl ? joinPath([this._skinUrl, icon]) : icon;
...
export default class Skin {
    ...
    async loadSkin(skinParam: string | SkinTheme) {
    ...
    unloadSkin() {
        // Unload custom icons
        if (Array.isArray(this._skinData?.icons)) {
            this._skinData?.icons.forEach(({ plugin, identifier } : { plugin: string, identifier: string }) => {
